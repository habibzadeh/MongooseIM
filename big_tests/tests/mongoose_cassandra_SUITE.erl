...
reset_all_cassandra_connections_loop(Interval) ->
    {ok, ProxyPid} = gun:open("localhost", 9191),
    Ref = gun:post(ProxyPid, <<"/routes/cassandra/links/close">>, []),
    Res = gun:await(ProxyPid, Ref),
...
ensure_rpc_is_possible() ->
    ...
    Node = escalus_ct:rpc_call(Node, erlang, node, [], 10000, Cookie).
...
call(M, F, A) ->
    Node = ct:get_config({hosts, mim, node}),
    Cookie = escalus_ct:get_config(ejabberd_cookie),
    escalus_ct:rpc_call(Node, M, F, A, timer:minutes(1), Cookie).
